...
//!
//! 用法：
//!   cargo run --release --example update_spike -- <url> <expected_size> [ext] [version] [sha256]
//!
//! 例：
...
//!     9013703 exe 0.1.2 sha256:...
//!
//! sha256 就是 GitHub Release API 里每个附件的 `digest` 字段（不带也行，就只做大小 + 文件头校验）。
//!
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("用法: update_spike <url> <expected_size> [ext] [version] [sha256]");
        std::process::exit(2);
    }
    ...
    let sha = args.get(5).cloned();

    println!("URL      : {url}");
    println!("期望大小 : {expected} 字节");
    println!("类型/版本: {ext} / {version}");
    println!("期望 sha : {}", sha.clone().unwrap_or_else(|| "（没给，只查大小和文件头）".into()));
    println!("开始下载…");

    let mut last_shown = 0u64;
    let result = fetch_update_package(&url, expected, sha.as_deref(), &ext, &version, |done, total| {
        // 每涨 10% 打一行，别刷屏
        if total > 0 {
