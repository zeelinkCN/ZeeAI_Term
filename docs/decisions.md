...
| M3 验收 | **上传/下载的进度条** | 文档写的是「上传/下载（带进度）」。现在只有「忙」状态，传大文件看不到进度，也没有断点续传。 |
...
1. **SFTP 其实是 scp**：上传/下载/改名/删除/新建目录都能用，也支持带空格的路径，但走的是系统
   `scp`（老 SCP 协议），不是 `russh-sftp`。所以没有进度、没有断点续传、没有并行。
2. **tmux 面板只有 列表 / attach / kill**：文档写的是「列表 / 新建 / attach / kill」，
   「新建」现在只能在「新建会话」对话框里做，面板里没有独立的新建入口。
3. **目录跟随只在 tmux 会话准**：非 tmux 的普通 shell 拿不到实时 cwd（只能退回家目录），
   文档里提过的「OSC 7 / shell 集成脚本」方案还没做。
